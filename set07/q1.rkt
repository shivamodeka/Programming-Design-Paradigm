...
(define (undefined-variables ae)
 (remove-dup(subdefined ae (list))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; subdefined : ArithmeticExpression StringList -> StringList
;; GIVEN: an arbitrary arithmetic expression and a string list
;; WHERE: the StringList contains the names of defined variables
;;        above the arithmetic expression
;; RETURNS: another list of the names of all undefined variables
;;          for the expression, without repetitions, in any order
;; STRATEGY: Use observer template for ArithmeticExpression

(define (subdefined exp lst)
  (cond
...
;; check-var : Variable StringList -> StringList
;; GIVEN: an arbitrary variable and a string list
;; WHERE: the StringList contains the names of defined variables
;;        above the use of the variable
;; RETURNS: another list with the variable name in the list if the variable
;;          is undefined
;; STRATEGY: Use the observer template for variable

(define (check-var var lst)
  (if (not (member (variable-name var) lst))
      (list (variable-name var)) empty))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; check-block : Block StringList -> StringList
;; GIVEN: an arbitrary block and a string list
;; WHERE: the StringList contains the names of defined variables
;;        above the block
;; RETURNS: another list with the variable name added in the list 
;;          if the variable in the block is undefined
;; STRATEGY: Use the observer template for block


(define (check-block ae lst)
  (append (subdefined (block-rhs ae) lst)
          (subdefined (block-body ae)
                      (if (not (member (variable-name (block-var ae)) lst))
                          (cons (variable-name (block-var ae)) lst) lst))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; check-call : Call StringList -> StringList
;; GIVEN: an arbitrary call and a string list
;; WHERE: the StringList contains the names of defined variables
;;        above the call
;; RETURNS: another list with the variable name in the list if the variable
;;          in the call is undefined
;; STRATEGY: Use the observer template for call


(define (check-call ae lst)
  (append (subdefined (call-operator ae) lst)
          (calls-variable (call-operands ae) lst)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; calls-variable : ArithmeticExpressionList StringList -> StringList
;; GIVEN: an arithmetic expression list and a StringList
;; WHERE: the StringList contains the names of defined variables
;;        above the call-operands
;; RETURNS: another list with the variable name in the list if the variable
;;          in the call-operands is undefined
;; EXAMPLES : see tests below
;; STRATEGY : use observer template of stringlist

#;(define (calls-variable ae lst)
  (cond
    [(empty? ae) empty]
    [else (append (subdefined (first ae) lst)
                  (calls-variable (rest ae) lst))]))

(define (calls-variable ae lst)
  (foldr
    (lambda (a b)
     (append (subdefined a lst) b)) empty ae))
;;TESTS
         ...
         (call (var "f")
                 (list (block (var "x")
                         ...
                         (block (var "y")
                            ...
                            (var "z"))))
(list "f" "x" "z")
"Should return the list (list f x z)")
  
  (check-equal?
   (undefined-variables (block (var "x")
                              (block (var "x")
                                     (var "x")
                                     (var "a"))
                              (block (var "y")
...
(list "x" "a")
"Should return the list (list x a)")
...
(list "z" "y")
"Should return the list (list z y)")
...
(list "x" "z")
"Should return the list (list x z)"))

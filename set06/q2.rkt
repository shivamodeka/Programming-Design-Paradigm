...
;; CONSTRCTOR TEMPLATE:
;; (make-world BallList Racket Color PosReal PosReal Boolean Boolean)
...
(define (initial-world y)
   ...
   (make-world (cons (make-ball INITIAL-X-POS INITIAL-Y-POS
                                ZERO ZERO) empty)
               (make-racket INITIAL-X-POS INITIAL-Y-POS ZERO
                            ZERO false ZERO ZERO)
                WHITE
                ZERO
                y
                true false)
...
(define (world-after-b w)  
  (if (world-is-rally? w)      
      ...
      (world-racket w) WHITE ZERO (world-simulation-speed w) false true)     
      w))
...
(define(racket-after-left r)
  (make-racket (racket-x r) (racket-y r)
               (- (racket-vx r) 1)(racket-vy r) (racket-selected? r)
...
(define (ball-list-after-serve bl)
  (map ball-after-serve bl))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; ball-after-serve : Ball -> Ball
;; RETURNS: the ball with the vx and vy components set to 3 and -9.
...
(define (ball-paused b)
  (make-ball (ball-x b) (ball-y b) ZERO ZERO))
...
(define (racket-paused r)
  (make-racket (racket-x r) (racket-y r) ZERO ZERO false (racket-px r)
               (racket-py r)))
...
(define (world-after-pause w)
  (make-world (cons (make-ball INITIAL-X-POS INITIAL-Y-POS ZERO
                               ZERO) empty)
                (make-racket INITIAL-X-POS INITIAL-Y-POS ZERO
                             ZERO false ZERO ZERO)
                WHITE
...
(define (ball-outbound? b)
   (>= (+(ball-y b)(ball-vy b)) CANVAS-HEIGHT))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; ball-inbound? Ball -> Boolean
;; GIVEN: a ball
;; RETURNS: true iff the ball does not hit the bottom wall

(define (ball-inbound? b)
   (not(ball-outbound? b)))
...
;; STRATEGY: Use HOF map lambda and filter on bl
...
(define (balls-after-tick bl r)
  (map
   ;;Ball -> Ball
   ;;RETURNS: each of the balls after the tick that are still in canvas
   (lambda (n) (ball-after-tick n r))
   (filter ball-inbound? bl)))
...
(define (right-wall-collision b)
  (make-ball
      (- CANVAS-WIDTH (- (+ (ball-x b) (ball-vx b)) CANVAS-WIDTH))
...
;; RETURNS: A list of balls after collision
;; STRATEGY: Use HOF map and lambda on bl
...
(define (balls-collided bl r)
  (map
   ;;Ball -> Ball
   ;;RETURNS: true iff the ball has collided with the racket
   (lambda (n) (check-collision? n r)) bl))
...
;; STRATEGY: cases on various collisions on racket
...
(define (racket-after-tick bl r)
  ...
  (cond
    ...
    [(>= (+(racket-x r) (racket-vx r)) RACKET-EDGE)
    (right-wall-racket r)]
     
    [(and (member true(balls-collided bl r)) (< (racket-vy r) ZERO))
    (racket-after-collision r)]
   
    [else (racket-without-collision r)])))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; left-wall-racket: Racket -> Racket
;; GIVEN: A racket 
;; RETURNS: A racket following the previous racket after the tick
;; EXAMPLES: (make-racket 330 384 1 0)= (make-racket 331 384 1 0 false 0 0)
;; STRATEGY: template of racket on r

(define (left-wall-racket r)
    (make-racket
    HALF-LENGTH (+ (racket-y r) (racket-vy r)) ZERO
    (racket-vy r) (racket-selected? r) (racket-px r) (racket-py r)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; selected-racket: Racket -> Racket
;; GIVEN: A racket 
;; RETURNS: A racket following the previous racket after the tick
;; EXAMPLES: (make-racket 330 384 1 0)= (make-racket 331 384 1 0 false 0 0)
;; STRATEGY: template of racket on r

(define (selected-racket r)
      (make-racket (racket-x r) (racket-y r)
...
;; right-wall-racket: Racket -> Racket
;; GIVEN: A racket
;; RETURNS: A racket following the previous racket after the tick
;; EXAMPLES: (make-racket 330 384 1 0)= (make-racket 331 384 1 0 false 0 0)
;; STRATEGY: template of racket on r

(define (right-wall-racket r)
  (make-racket
   RACKET-EDGE (+ (racket-y r) (racket-vy r)) ZERO
   (racket-vy r) (racket-selected? r) (racket-px r) (racket-py r)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; racket-after-collision: Racket -> Racket
;; GIVEN: A racket 
;; RETURNS: A racket following the previous racket after the tick
;; EXAMPLES: (make-racket 330 384 1 0)= (make-racket 331 384 1 0 false 0 0)
;; STRATEGY: template of racket on r

 (define (racket-after-collision r)
   (make-racket (+ (racket-x r) (racket-vx r)) (racket-y r) (racket-vx r) ZERO
               (racket-selected? r) (racket-px r) (racket-py r)))
...
(define (racket-without-collision r)
  (make-racket
   ...
   (racket-vx r) (racket-vy r) (racket-selected? r) (racket-px r)
   (racket-py r)))
...
(check-equal?
 (world-after-tick (make-world (list (make-ball 329 381 3 2))
                               (make-racket 330 384 0 -1 false 0 0)
                               WHITE 0 0.05 false true))
                  (make-world (list (make-ball 332 383 3 -3))
                              (make-racket 330 384 0 0 false 0 0)
                              "white" 0 1/20 #f #t)
                  "Ball's vy component should change after the tick")
